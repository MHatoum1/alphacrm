// src/components/pages/EntityNoTabNoSearchPage.tsx
...
import CustomDataGridNoSearch from "@/components/ui/CustomDataGridNoSearch";
import { GridState } from "@/components/ui/DataTablesMapper";
import { useTranslation } from "react-i18next";

export interface EntityNoTabNoSearchPageProps<
  FetchArg = any,
  RawRow = any,
  Row = any
> {
  ...
  buildFetchArg: (opts: { grid: GridState }) => FetchArg;
...
export default function EntityNoTabNoSearchPage<FetchArg, RawRow, Row>({
  fetchThunk,
  selectSlice,
  mapRow,
  buildColumns,
  ...
  PaperProps,
}: EntityNoTabNoSearchPageProps<FetchArg, RawRow, Row>) {
  // ─── hooks & state ─────────────────────────
  ...
  const prevGrid = useRef<GridState>(gridState);
  useEffect(() => {
    if (gridState.columns.length === 0) return;
    if (JSON.stringify(prevGrid.current) === JSON.stringify(gridState)) return;
    prevGrid.current = gridState;
    dispatch(fetchThunk(buildFetchArg({ grid: gridState })));
  }, [gridState, dispatch, fetchThunk, buildFetchArg]);
