// src/components/pages/EntityNoTabNoTitlePage.tsx
...
import CustomDataGridWithOnlySearch from "@/components/ui/CustomDataGridWithOnlySearch";
import { GridState } from "@/components/ui/DataTablesMapper";
import { useTranslation } from "react-i18next";

export interface EntityNoTabNoTitlePageProps<
  FetchArg = any,
  RawRow = any,
  Row = any
> {
...
export default function EntityNoTabNoTitlePage<FetchArg, RawRow, Row>({
  fetchThunk,
  selectSlice,
  mapRow,
  buildColumns,
  ...
  PaperProps,
}: EntityNoTabNoTitlePageProps<FetchArg, RawRow, Row>) {
  // ─── hooks & state ─────────────────────────
  ...
  // ─── render ─────────────────────────────────
  return (
    <Paper sx={{ boxShadow: "none" }} {...PaperProps}>
      {/* data grid */}
      <Box>
        <CustomDataGridWithOnlySearch
          rows={rows}
          columns={uiColumns}
