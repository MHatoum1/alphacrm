// src/components/pages/EntityNoTabAutoHeightPage.tsx
...
import { renderDetails } from "@/pages/Admin/Details/Logs";

export interface EntityNoTabAutoHeightPageProps<
  FetchArg = any,
  RawRow = any,
  Row = any
> {
...
export default function EntityNoTabAutoHeightPage<FetchArg, RawRow, Row>({
  fetchThunk,
  selectSlice,
  mapRow,
  buildColumns,
  ...
  PaperProps,
}: EntityNoTabAutoHeightPageProps<FetchArg, RawRow, Row>) {
  // ─── hooks & state ─────────────────────────
    ...
    <Paper sx={{ p: 2, boxShadow: "none" }} {...PaperProps}>
      ...
      <Box>
        <CustomDataGrid
          ...
          // ← these two props let each row grow to fit its content
          autoHeight
          getRowHeight={() => "auto"}
          // add 8px above & below each row
          gridExtraProps={{
            sx: {
              // top-align and pad each cell more heavily
              "& .MuiDataGrid-cell": {
        ...
        <DialogContent dividers>
          {dialogRow &&
            ...
            desktopColumns.map((col) => {
              ...
              // __ Special‐case “details” to use our mobile parser: __
              if (field === "details") {
                const html = (dialogRow.details as string) || "";
                return (
                  <Box key={field} sx={{ mb: 1 }}>
                    ...
                    {html ? (
                      renderDetails(html, true /* isMobile */)
                    ) : (
              ...
              // fall back to normal for date/action
              let val = dialogRow[field];
              if (val == null || val === "") val = "--";
              return (
                <Box key={field} sx={{ mb: 1 }}>
