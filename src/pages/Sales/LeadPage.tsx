...
export default function LeadPage() {
  ...
  const handleComm = () =>
    dispatch(saveCommMethod({ user_id, id: +id!, method: commDraft }));
  const handleStat = () =>
    dispatch(saveClientStatus({ user_id, id: +id!, status: statusDraft }));
  ...
  const Badge = ({ ok, label }: { ok: boolean; label: string }) => (
    ...
    >
      {ok ? (
        ...
        <CloseIcon sx={{ fontSize: 16, color: "#c62828" }} />
      )}
      <span style={{ color: ok ? "#2e7d32" : "#c62828" }}>{label}</span>
    </Stack>
  );
        ...
        <Grid container spacing={1} alignItems="center" mb={2}>
          ...
          <Grid item xs={12} sm={2}>
            <Stack direction="row" spacing={1} alignItems="center">
              <Badge ok={!!lead.documents} label={t("documents")} />
              <IconButton
                size="small"
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          ...
          <Grid item xs={12} sm={2}>
            <Button
              ...
              onClick={() =>
                dispatch(revertLead({ user_id: user_id, id: +id! }))
              }
            >
              {t("revert_to_admin")}
            </Button>
          ...
          <Grid item xs={12} sm={2}>
            <Button
              ...
              variant="contained"
              disabled={actionStatus === "loading"}
              onClick={() => dispatch(makeCold({ user_id, id: +id! }))}
            >
              {t("cold_leads")}
            </Button>
          </Grid>
          ...
          <Grid item xs={12} sm={2}>
            <Button
              ...
              variant="contained"
              component="a"
              href={`/leads/transactions_history/${id}`}
              target="_self"
              rel="noopener"
            >
          ...
          {/* —— Demo ⇄ All phase toggle —— */}
          <Grid item xs={12} sm={2}>
            <Button
              fullWidth
              sx={{
                color: "#fff",
                bgcolor: lead.phase === "demo" ? "#d32f2f" : "#2e7d32",
                "&:hover": {
                  bgcolor: lead.phase === "demo" ? "#b71c1c" : "#1b5e20",
                },
              }}
              disabled={phaseToggling === "loading"}
              onClick={() => dispatch(toggleDemoPhase({ user_id, id: +id! }))}
            >
              {lead.phase === "demo"
                ? t("unset_demo_phase")
                : t("set_demo_phase")}
