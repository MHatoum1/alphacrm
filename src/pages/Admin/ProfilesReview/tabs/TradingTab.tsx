// src/pages/Admin/ProfilesReview/tabs/TradingTab.tsx
import {
  Box,
  Grid,
  Paper,
    ...
    <Controller
      ...
      render={({ field, fieldState }) => (
        <FormControl fullWidth size="small" error={!!fieldState.error}>
          ...
          {fieldState.error && <FormHelperText>{t("required")}</FormHelperText>}
        </FormControl>
      )}
    />
  );
...
export default function TradingTab() {
  ...
  } = useForm<TradingForm>({ defaultValues: defaults });
  useEffect(() => reset(defaults), [defaults, reset]);

  const exp = useWatch({
    control,
    name: "experience",
    defaultValue: defaults.experience, // ← make sure RHF knows your initial value
  });
  const onSubmit = async (vals: TradingForm) => {
    if (!id) return;
    try {
      await dispatch(
        updateProfile({ id, section: "trading", fields: vals })
      ).unwrap();
  ...
  /* ─── render no-exp checkboxes with validation ─── */
  const renderNoExpCheckboxes = ({ field, fieldState }: any) => {
    ...
    <Box>
      ...
      >
        ...
        <Grid container rowSpacing={2} columnSpacing={4}>
          ...
          {exp == "0" && (
            <Grid item xs={12}>
              <FormLabel required>{t("experience_no_exp")}</FormLabel>
              <Controller
                name="no_exp"
                control={control}
                rules={{
                  validate: (arr: string[]) => arr.length > 0 || t("required"),
                }}
          ...
          <Grid item xs={12}>
              ...
              <Controller
                ...
                control={control}
                rules={{ required: true }}
                defaultValue={defaults.nature_of_risk}
                render={({ field, fieldState }) => (
                  <>
                    <RadioGroup
                      row
