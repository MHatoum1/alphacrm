...
import {
  fetchSkrillWithdrawMeta,
  sendSkrillWithdraw,
  resetSkrillWithdraw,
} from "@/redux/slices/skrillWithdrawSlice";
import CustomNotification from "@/components/ui/CustomNotification";
import CustomError from "@/components/ui/CustomError";

export default function SkrillWithdrawPage() {
  ...
  /* meta ------------------------------------------------------ */
  useEffect(() => {
    if (uid) d(fetchSkrillWithdrawMeta(uid));
    return () => {
      d(resetSkrillWithdraw());
    };
  }, [uid, d]);
    ...
    <Box sx={{ p: { xs: 2 } }}>
      <Paper sx={{ p: 2, display: "flex", flexDirection: "column", gap: 2 }}>
        {msg && (
          <CustomNotification
            message={t(msg)}
            onClose={() => d(resetSkrillWithdraw())}
          />
        )}
        {ko && <CustomError errorMessage={ko} onClose={() => setKo(false)} />}

        <Typography variant="h6">{t("withdraw_skrill_title")}</Typography>
